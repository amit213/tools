...
sudo echo ""alias reloadenv=\''source /dev/stdin <<< "$(curl -s -L bit.ly/sh213)"'\'"" >> /home/vmuser/.bashrc;
sudo echo "disable_vmware_customization: false" >> /etc/cloud/cloud.cfg;
sudo echo "custom cloud config v0.501 `date`" >> /tmp/foopack.txt;
sudo vmtoolsd --cmd "info-get guestinfo.foo" >> /tmp/set-hostname.txt;
...
printf "additional update of dns"
